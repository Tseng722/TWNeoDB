...
class Command(BaseCommand):
    ...
    # python manage.py save_mtsa_only_dna /work1791/cindy2270/TSA_final/oral103/final/final_mtsa_DNA_oral103_004.csv oral103 oral 004
    def add_arguments(self, parser):
        ...
        parser.add_argument('RNA', type=str, help='A True/False flag')
                ...
                if not patient_transcript_score.objects.filter(dna_id = dna_id_instance,patient_id = patient_info_instance).exists():
                    if RNA == False:
                        ...
                        patient_transcript_score.objects.create(
                            ...
                            tumor_dna_depth = df.at[i,'Tumor DNA Depth'],
                            tumor_dna_vaf = df.at[i,'Tumor DNA VAF'],
                            tpm_normal = df.at[i,'tpm_normal'] if not pd.isna(df.at[i,'tpm_normal']) else None,
                            tpm_tumor = df.at[i,'tpm_tumor'] if not pd.isna(df.at[i,'tpm_tumor']) else None,
                            dna_id = dna_id_instance,
                            patient_id = patient_info_instance
                        )
